    ...
    private static void RefFixPatch_ZNetSceneAwake(ZNetScene __instance)
    {
        foreach (GameObject prefab in piecePrefabs.Concat(ZnetOnlyPrefabs).Concat(customPiecePrefabs.Keys))
        {
            if (__instance.m_prefabs.Contains(prefab))
            {
                ...
                prefab.GetComponent<Piece>().m_placeEffect = __instance.GetPrefab("piece_workbench_ext3").GetComponent<Piece>().m_placeEffect;
                prefab.GetComponent<WearNTear>().m_destroyedEffect = __instance.GetPrefab("piece_workbench_ext3").GetComponent<WearNTear>().m_destroyedEffect;
                prefab.GetComponent<WearNTear>().m_hitEffect = __instance.GetPrefab("piece_workbench_ext3").GetComponent<WearNTear>().m_hitEffect;
            }
        }
    }
